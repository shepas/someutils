...
class EpicrisTemplates():
    ...
    def addTemplate(self, name, code, orgStruct, htmlTemplate, epicType, actionType):
        ...
        if not recordTemplate:
            ...
            rec.setValue('actionType_id', actionType)
            QtGui.qApp.db.insertRecord(tableTemplate, rec)

    # Редактирование шаблона эпикриза
    def modifyTemplate(self, epicId, name, code, orgStruct, htmlTemplate, epicType, actionType):
        tableTemplate = QtGui.qApp.db.table('rbEpicrisisTemplates')
        ...
        recordTemplate.setValue('actionType_id', actionType)
    ...
    def addEpicrisTemplate(self, docum, modify=False):
        ...
        while not epicList.isNull():
            ...
            if epicElement:
                ...
                epicActType = forceString(epicElement.firstChildElement('ID_ACTIONTYPE').text())
            if modify:
                self.modifyTemplate(epicId, epicName, epicCode, epicOrgStruct, epicPrintTemplate, epicType, epicActType)
            else:
                self.addTemplate(epicName, epicCode, epicOrgStruct, epicPrintTemplate, epicType, epicActType)
